// Admission feeder for lake-ontario — a PRIVATE background worker.
// Reaches its lake ONLY through the system wire (LAKE_URL).
const http = require('http')
const NAMES = 'Ava,Liam,Emma,Noah,Olivia,Ethan'.split(',')
let sent = 0
setInterval(async () => {
...
http.createServer((req, res) => {
  ...
  res.end(JSON.stringify(req.url === '/health' ? { status: 'ok' } : { feeder: 'feeder-ontario', sent }))
