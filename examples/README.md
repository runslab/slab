...
| `waffle-house/` | a 3-tier web app over pg-cluster; the waffle move (`node =` spans machines) | 2× `Dockerfile` + pg-cluster by source | services | brings its own | Public web → private api → pgbouncer; uncomment one `node =` line and the web tier runs on a peer, trunk conceals the distance. `slab up examples/waffle-house`. |
